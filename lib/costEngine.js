 ...
 * CRITICAL dedup rule: skip records where stop_reason is null or undefined —
 * these are streaming-start duplicates that would double all token counts.
 *
...
export function extractUsageRecords(jsonlRecords) {
  ...
  for (const record of jsonlRecords) {
    ...
    // Dedup rule: skip streaming-start records (stop_reason null/undefined doubles counts)
