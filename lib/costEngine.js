...
export function getContextFillPercent(model, lastUsage) {
  // Context fill = how full the input window is. Only count tokens that actually
  // consume the context window: raw input + cache read.
  // Do NOT include cacheWrite tokens (thinking tokens) — these are generated
  // output that gets cached; they inflate the % during thinking generation
  // and cause it to drop when thinking is printed to the conversation.
