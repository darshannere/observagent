...
async function findGitRootName(cwd) {
  let dir = cwd;
  for (let i = 0; i < 100; i += 1) {
    if (!dir || dir.length < 2) break;
    try {
 ...
 * Extracts the project name using cwd.
 * Prefers git repo root name when available, otherwise falls back to cwd basename.
 */
async function extractProjectName(rawRecords) {
  for (const r of rawRecords) {
    if (r.cwd && typeof r.cwd === 'string' && r.cwd.length > 1) {
      const cached = projectNameCache.get(r.cwd);
      if (cached) return cached;
      const repoName = await findGitRootName(r.cwd);
      const projectName = repoName ?? basename(r.cwd);
      projectNameCache.set(r.cwd, projectName);
      return projectName;
    }
  }
  return 'unknown';
      ...
      for (const row of emptyRows) {
        ...
        for (const dir of backfillProjectDirs) {
          ...
          try {
            ...
            const pn = await extractProjectName(recs);
