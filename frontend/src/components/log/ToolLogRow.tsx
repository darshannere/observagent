...
export function ToolLogRow({ event }: ToolLogRowProps) {
  ...
  const rowClass = [
    'px-1 py-0.5 font-mono text-xs border-l-2 flex flex-col gap-0 hover:bg-primary/[0.03]',
    isError
    ...
    <div className={rowClass}>
      ...
      <div className="flex items-center gap-2 min-w-0">
        <span className={[
          ...
          isError
            ? 'bg-destructive shadow-[0_0_5px_rgba(255,77,77,0.5)]'
            : isInProgress
              ? 'bg-primary animate-pulse'
              : 'bg-green',
        ].join(' ')} />
        <span className="text-primary font-mono font-semibold truncate shrink-0 max-w-[140px]">
          {event.tool_name}
        </span>
